window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 插件样式（固定类名前缀 dsm-）。 */
		var css = [
			".dsm-line{display:flex;align-items:center;gap:8px;min-height:24px;box-sizing:border-box;padding:0 28px 8px 20px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);font-family:var(--dsw-font-family);font-variant-numeric:tabular-nums}",
		...
		function SystemMonitorLine() {
			...
			react.useEffect(function () {
				...
				if (!el) return;
				var utilities = el.parentElement;
				var titleRow = utilities && utilities.parentElement;
				if (titleRow && el.parentNode !== titleRow.parentNode) {
					titleRow.parentNode.insertBefore(el, titleRow.nextSibling);
				}
			}, []);
