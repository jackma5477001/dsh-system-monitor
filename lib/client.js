window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 分上下两行显示在右上角：上方是 Session log / 余额，下方是
		 * DSH 所在电脑的实时性能（CPU 使用率与内存使用率）。
		 *
		 * 注册到 `conversation.session.header.utilities` list slot（与
		 * session-log-download、deepseek-balance 同一 slot），通过网格布局
		 * 让这两个控件保持上行、监控信息保持下行，并统一右对齐。
		 *
		...
		/** 插件样式（固定类名前缀 dsm-）。 */
		var css = [
			".dsm-line{display:flex;grid-column:1/-1;justify-self:end;align-items:center;gap:8px;min-height:24px;box-sizing:border-box;padding:0;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);font-family:var(--dsw-font-family);font-variant-numeric:tabular-nums}",
		...
		function SystemMonitorLine() {
			...
			// headerUtilities 默认是横向 flex；改成两列网格，让已有的
			// Session log / 余额占据第一行，监控行占据第二行并右对齐。
			react.useEffect(function () {
				var el = rootRef.current;
				var slot = el && el.parentElement;
				var utilities = slot && slot.parentElement;
				if (!utilities || !utilities.style) return;

				var style = utilities.style;
				var previous = {
				...
				return function () {
					...
					style.gridTemplateColumns = previous.gridTemplateColumns;
					style.justifyContent = previous.justifyContent;
					style.alignItems = previous.alignItems;
					style.gap = previous.gap;
				};
			}, []);
